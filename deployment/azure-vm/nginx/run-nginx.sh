#!/bin/sh
# Substitutes env vars into the nginx config and waits for the API server
# before starting nginx. Run from container with config name as first arg
# (e.g. app.conf.eleven.prod). Requires .env.nginx for DOMAIN etc.

# fill in the config from env
...
if [ "${MCP_SERVER_ENABLED}" = "True" ] || [ "${MCP_SERVER_ENABLED}" = "true" ]; then
  ...
  envsubst '$ONYX_MCP_SERVER_HOST' < "/etc/nginx/conf.d/mcp_upstream.conf.inc.eleven" > /etc/nginx/conf.d/mcp_upstream.conf.inc
  envsubst '$ONYX_MCP_SERVER_HOST' < "/etc/nginx/conf.d/mcp.conf.inc.eleven" > /etc/nginx/conf.d/mcp.conf.inc
else
