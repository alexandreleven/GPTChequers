...
class NotionSharepointConnector(
    ...
    def _normalize_sharepoint_url(url: str) -> tuple[str | None, list[str]]:
        ...
        path_segments = NotionSharepointConnector._strip_share_link_tokens(parsed.path)
    ...
    def _extract_site_and_drive_info(site_urls: list[str]) -> list[SiteDescriptor]:
        ...
        for url in site_urls:
            base_url, parts = NotionSharepointConnector._normalize_sharepoint_url(
                url.strip()
            )
            if base_url is None:
                continue
    ...
    def fetch_sites(self) -> list[SiteDescriptor]:
        ...
        # OneDrive personal sites should not be indexed with NotionSharepointConnector
        site_descriptors = [
            SiteDescriptor(
                url=site.web_url or "",
                drive_name=None,
...
if __name__ == "__main__":
    ...
    connector = NotionSharepointConnector(
        sites=os.environ["SHAREPOINT_SITES"].split(",")
    )
